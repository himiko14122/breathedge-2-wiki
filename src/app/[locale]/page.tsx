...
import { ZONES, STAR_SYSTEMS, CREW, CRAFTING, TIER_COLOR_MAP } from '@/data/game-data';
import AdBanner from '@/components/AdBanner';
import AutoScrollCarousel from '@/components/AutoScrollCarousel';
import YouTubePlayer from '@/components/YouTubePlayer';
import { ArrowRight, ChevronRight, Globe2, Users, Hammer, Rocket } from 'lucide-react';
...
export default async function HomePage({ params }: { params: Promise<{ locale: string }> }) {
  ...
  return (
    <div>
      ...
      {/* ===== ZONES TABLE (Data-Driven Table 1) ===== */}
      <section className="bg-[var(--color-bg-secondary)] border-y border-[var(--color-border)] full-width-bg">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
          <div className="flex items-center justify-between mb-8">
            <div>
              <div className="section-label">{t('home_module_gameplay')}</div>
              <h2 className="text-2xl md:text-3xl font-bold font-[var(--font-heading)] gradient-text">{t('home_zone_table_title')}</h2>
            </div>
            <Link href="/gameplay" title={t('home_module_gameplay')} className="text-sm font-semibold text-[var(--color-accent)] hover:underline flex items-center gap-1">{t('view_all')} <ArrowRight className="w-3 h-3" /></Link>
          </div>
          <p className="text-[var(--color-text-secondary)] mb-6 leading-relaxed">{t('home_zone_table_desc')}</p>
          <div className="rounded-2xl bg-white/[0.02] border border-white/[0.08] overflow-hidden backdrop-blur-sm">
            <div className="grid grid-cols-[2rem_1fr_5rem_2fr_2fr] sm:grid-cols-[2.5rem_1.5fr_6rem_2fr_2fr] gap-x-4 text-xs font-semibold text-[var(--color-text-muted)] uppercase tracking-wider border-b border-[var(--color-border)] px-4 py-3">
              <span></span><span>{t('home_zone_name')}</span><span>{t('tierList_tierLabel')}</span><span>{t('home_zone_type')}</span><span>{t('home_zone_threat')}</span>
            </div>
            {ZONES.map((zone, idx) => {
              const tc = tierColorVal(zone.tier);
              return (
                <Link key={zone.id} href="/gameplay" title={t(zone.nameKey)} className="grid grid-cols-[2rem_1fr_5rem_2fr_2fr] sm:grid-cols-[2.5rem_1.5fr_6rem_2fr_2fr] items-center gap-x-4 px-4 py-3 border-b border-white/[0.04] last:border-b-0 hover:bg-[var(--color-accent)]/5 transition-colors group">
                  ...
                  <span className="justify-self-start text-[10px] font-bold px-1.5 py-0.5 rounded-full" style={{ color: tc, background: `${tc}15` }}>{zone.tier}</span>
                  <span className="text-xs text-[var(--color-text-secondary)]">{t(zone.typeKey)}</span>
                  <span className="text-xs text-[var(--color-text-secondary)]">{t(zone.threatKey)}</span>
                </Link>
              );
            })}
          </div>
      ...
      {/* ===== CREW CARDS (Data-Driven Cards 1) ===== */}
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
        <div className="flex items-center justify-between mb-8">
          <div>
            <div className="section-label">{t('home_module_crew')}</div>
            <h2 className="text-2xl md:text-3xl font-bold font-[var(--font-heading)] gradient-text">{t('home_crew_cards_title')}</h2>
          </div>
          <Link href="/crew" title={t('home_module_crew')} className="text-sm font-semibold text-[var(--color-accent)] hover:underline flex items-center gap-1">{t('view_all')} <ArrowRight className="w-3 h-3" /></Link>
        </div>
        <p className="text-[var(--color-text-secondary)] mb-6 leading-relaxed">{t('home_crew_cards_desc')}</p>
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-5">
          {CREW.map((member) => {
            const tc = tierColorVal(member.tier);
            return (
              <Link key={member.id} href="/crew" title={t(member.nameKey)} className="category-card group block rounded-2xl overflow-hidden">
                <div className="p-5">
                  <div className="flex items-center gap-2 mb-3">
                    <Users className="w-4 h-4 text-[var(--color-accent)]" />
                    <span className="text-[10px] font-bold px-2 py-0.5 rounded-full" style={{ color: tc, background: `${tc}15` }}>{member.tier}</span>
                  </div>
                  <h3 className="text-[0.9375rem] font-bold font-[var(--font-heading)] text-[var(--color-text-primary)] group-hover:text-[var(--color-accent)] transition-colors mb-1">{t(member.nameKey)}</h3>
                  <p className="card-meta text-[0.75rem] text-[var(--color-text-secondary)] mb-2"><span className="meta-label">{t('home_crew_role')}: </span><span className="meta-value">{t(member.roleKey)}</span></p>
                  <p className="card-desc text-[0.8125rem] text-[var(--color-text-secondary)]">{t('home_crew_need')}: {t(member.needKey)}</p>
                </div>
              </Link>
            );
          })}
      ...
      {/* ===== CRAFTING TABLE (Data-Driven Table 2) ===== */}
      <section className="bg-[var(--color-bg-secondary)] border-y border-[var(--color-border)] full-width-bg">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
          <div className="flex items-center justify-between mb-8">
            <div>
              <div className="section-label">{t('home_module_crafting')}</div>
              <h2 className="text-2xl md:text-3xl font-bold font-[var(--font-heading)] gradient-text">{t('home_craft_table_title')}</h2>
            </div>
            <Link href="/crafting" title={t('home_module_crafting')} className="text-sm font-semibold text-[var(--color-accent)] hover:underline flex items-center gap-1">{t('view_all')} <ArrowRight className="w-3 h-3" /></Link>
          </div>
          <p className="text-[var(--color-text-secondary)] mb-6 leading-relaxed">{t('home_craft_table_desc')}</p>
          <div className="rounded-2xl bg-white/[0.02] border border-white/[0.08] overflow-hidden backdrop-blur-sm">
            <div className="grid grid-cols-[2rem_1fr_5rem_2fr_2fr] sm:grid-cols-[2.5rem_1.5fr_6rem_2fr_2fr] gap-x-4 text-xs font-semibold text-[var(--color-text-muted)] uppercase tracking-wider border-b border-[var(--color-border)] px-4 py-3">
              <span></span><span>{t('home_craft_name')}</span><span>{t('tierList_tierLabel')}</span><span>{t('home_craft_source')}</span><span>{t('home_craft_use')}</span>
            </div>
            {CRAFTING.map((craft, idx) => {
              const tc = tierColorVal(craft.tier);
              return (
                <Link key={craft.id} href="/crafting" title={t(craft.nameKey)} className="grid grid-cols-[2rem_1fr_5rem_2fr_2fr] sm:grid-cols-[2.5rem_1.5fr_6rem_2fr_2fr] items-center gap-x-4 px-4 py-3 border-b border-white/[0.04] last:border-b-0 hover:bg-[var(--color-accent)]/5 transition-colors group">
                  <span className="text-xs font-mono text-[var(--color-text-muted)]">{idx + 1}</span>
                  <span className="flex items-center gap-2 font-semibold text-sm group-hover:text-[var(--color-accent)] transition-colors">
                    <Hammer className="w-4 h-4 text-[var(--color-accent)]" />
                    {t(craft.nameKey)}
                  </span>
                  <span className="justify-self-start text-[10px] font-bold px-1.5 py-0.5 rounded-full" style={{ color: tc, background: `${tc}15` }}>{craft.tier}</span>
                  <span className="text-xs text-[var(--color-text-secondary)]">{t(craft.sourceKey)}</span>
                  <span className="text-xs text-[var(--color-text-secondary)]">{t(craft.useKey)}</span>
                </Link>
              );
            })}
          </div>
      ...
      {/* ===== STAR SYSTEM CARDS (Data-Driven Cards 2) ===== */}
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
        <div className="flex items-center justify-between mb-8">
          <div>
            <div className="section-label">{t('home_module_spaceships')}</div>
            <h2 className="text-2xl md:text-3xl font-bold font-[var(--font-heading)] gradient-text">{t('home_sys_cards_title')}</h2>
          </div>
          <Link href="/spaceships" title={t('home_module_spaceships')} className="text-sm font-semibold text-[var(--color-accent)] hover:underline flex items-center gap-1">{t('view_all')} <ArrowRight className="w-3 h-3" /></Link>
        </div>
        <p className="text-[var(--color-text-secondary)] mb-6 leading-relaxed">{t('home_sys_cards_desc')}</p>
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5">
          {STAR_SYSTEMS.map((sys) => {
            const tc = tierColorVal(sys.tier);
            return (
              <Link key={sys.id} href="/spaceships" title={t(sys.nameKey)} className="category-card group block rounded-2xl">
                <div className="flex items-center gap-2 mb-2">
                  <span className="text-[10px] font-bold px-2 py-0.5 rounded-full" style={{ color: tc, background: `${tc}15` }}>{sys.tier}</span>
                  <Rocket className="w-4 h-4 text-[var(--color-accent)]" />
                </div>
                <h3 className="text-[0.9375rem] font-bold font-[var(--font-heading)] text-[var(--color-text-primary)] group-hover:text-[var(--color-accent)] transition-colors mb-1">{t(sys.nameKey)}</h3>
                <p className="card-meta text-[0.75rem] text-[var(--color-text-secondary)] mb-2"><span className="meta-label">{t('home_sys_role')}: </span><span className="meta-value">{t(sys.roleKey)}</span></p>
                <p className="card-desc text-[0.8125rem] text-[var(--color-text-secondary)]">{t('home_sys_resource')}: {t(sys.resourceKey)}</p>
              </Link>
            );
          })}
        </div>
